...
export default function VedettesClient({
  ...
  async function addProduct(product: VedetteProduct) {
    if (selectedProductIds.has(product.id)) {
      setNotice('Ce produit est deja dans la selection.')
      return
    }
    if (vedettes.length >= MAX_VEDETTES) {
      setNotice(`Maximum ${MAX_VEDETTES} produits.`)
      return
    }
    ...
    try {
      ...
      setNotice('Produit ajoute a Meilleures ventes.')
    } catch (error) {
      console.error(error)
      setNotice('Echec de l ajout du produit.')
    } finally {
  ...
  async function removeProduct(vedette: Vedette) {
    ...
    try {
      ...
      setNotice('Produit retire de Meilleures ventes.')
    } catch (error) {
      console.error(error)
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="mb-8">
        <h1 className="mb-2 text-4xl font-bold text-blue-600">Vedettes</h1>
        <p className="text-lg text-slate-600">
          Selectionnez jusqu a {MAX_VEDETTES} produits pour la section Meilleures ventes de la page d accueil.
        </p>
      </div>
      ...
      <div className="mb-6 flex items-center justify-between rounded-lg border border-slate-200 bg-white px-4 py-3">
        <span className="text-sm font-medium text-slate-700">Selection actuelle</span>
      ...
      <section>
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-slate-800">Ajouter un produit</h2>
        </div>
