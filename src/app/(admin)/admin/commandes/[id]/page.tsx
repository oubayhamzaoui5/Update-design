...
async function getOrderDetail(id: string) {
  ...
  try {
    const record = await pb.collection('Commandes').getOne(id, { requestKey: null })
    ...
    const items = await Promise.all(
      rawItems.map(async (item: Record<string, unknown>) => {
        ...
        if (productId) {
          try {
            const product = await pb.collection('Produits').getOne(productId, {
              fields: 'id,images',
              requestKey: null,
            })
    ...
    return {
      ...
      status: String(record.status ?? 'Payee') as OrderStatus,
          ...
          <div className="rounded-2xl bg-white p-5" style={{ border: '1px solid #E8EAED', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            ...
            <form
              action={async (formData: FormData) => {
                'use server'
                const status = formData.get('Statut') as string
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl bg-white p-5" style={{ border: '1px solid #E8EAED', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            ...
            <div className="space-y-2 text-sm">
              ...
              {order.userId && (
                <Link
                  href={`/admin/utilisateurs?search=${encodeURIComponent(order.email || order.firstName)}`}
          ...
          <div className="rounded-2xl bg-white p-5" style={{ border: '1px solid #E8EAED', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            ...
            <p className="text-sm" style={{ color: '#6B7280' }}>
              {order.paymentMode === 'cash_on_delivery' ? 'Paiement a la livraison' : order.paymentMode}
            </p>
          </div>
        </div>
      </div>
