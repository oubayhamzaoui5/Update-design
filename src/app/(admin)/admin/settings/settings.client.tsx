...
export default function SettingsClient({ user }: { user: AdminUser }) {
  ...
  const fullName = `${user.surname ?? ''} ${user.name ?? ''}`.trim() || 'Aucun nom'
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!result.success) {
        setStatus({ type: 'error', msg: result.error ?? 'Echec de la mise a jour du mot de passe.' })
        return
      }
      ...
      setNewPassword('')
      setConfirmPassword('')
      setStatus({ type: 'success', msg: 'Mot de passe mis a jour avec succes.' })
    } catch {
      setStatus({ type: 'error', msg: 'Une erreur inattendue est survenue.' })
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="mx-auto max-w-2xl px-6 py-8">
      ...
      <div className="mb-6 rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-4 px-6 py-5 sm:grid-cols-2">
          <InfoRow icon={<UserRound size={14} />} label="Nom complet" value={fullName} />
          <InfoRow icon={<Shield size={14} />} label="Role" value={user.role === 'admin' ? 'Admin' : 'Client'} />
          <div className="sm:col-span-2">
            <InfoRow icon={<Mail size={14} />} label="Email" value={user.email || 'Aucun e-mail'} />
          </div>
        </div>
      </div>
