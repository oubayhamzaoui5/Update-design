    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Rechercher par nom, e-mail, telephone..."
            value={query}
      ...
      <div className="overflow-x-auto rounded-2xl bg-white" style={{ border: '1px solid #E8EAED', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-4 py-12 text-center" style={{ color: '#9CA3AF' }}>Aucun utilisateur trouve.</td>
              </tr>
            ) : (
              filtered.map(user => (
                <tr key={user.id} className="transition-colors hover:bg-[#F9FAFB]" style={{ borderBottom: '1px solid #F0F2F5' }}>
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      title={user.role === 'admin' ? 'Impossible de desactiver un administrateur' : user.isActive ? 'Desactiver' : 'Activer'}
                    ...
                    >
                      {togglingId === user.id ? '...' : user.isActive ? 'Desactiver' : 'Activer'}
                    </button>
                  </td>
                </tr>
              ))
