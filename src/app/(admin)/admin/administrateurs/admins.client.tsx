...
import type { AdminUser } from '../utilisateurs/page'
import { createAdminUserAction, resetAdminPasswordAction } from '../utilisateurs/actions'
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        ...
        <form onSubmit={handleCreateAdmin} className="grid grid-cols-1 gap-3 md:grid-cols-4">
          ...
          >
            {creating ? 'Creation...' : 'Creer un administrateur'}
          </button>
        </form>
      </div>
      ...
      <div className="overflow-x-auto rounded-2xl bg-white" style={{ border: '1px solid #E8EAED', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-4 py-12 text-center" style={{ color: '#9CA3AF' }}>Aucun administrateur trouve.</td>
              </tr>
            ) : (
              filtered.map((admin) => (
                <tr key={admin.id} className="transition-colors hover:bg-[#F9FAFB]" style={{ borderBottom: '1px solid #F0F2F5' }}>
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      title={canManageAdmins ? 'Reinitialiser le mot de passe administrateur' : 'Seul admin@admin.com peut reinitialiser le mot de passe'}
                    >
                      <KeyRound size={12} />
                      {resettingId === admin.id ? 'Reinitialisation...' : 'Reinitialiser le mot de passe'}
                    </button>
                  </td>
                </tr>
              ))
