...
import { Trash2, ChevronDown, ChevronUp, Pause, ShoppingCart, CheckCircle2, Clock, Truck, Search } from 'lucide-react'
...
export default function OrdersClient({ initialOrders }: { initialOrders: OrderRecord[] }) {
  ...
  async function deleteOrder(id: string) {
    ...
    try {
      ...
      setNotice('Commande supprimee.')
      window.dispatchEvent(new Event('admin:orders-changed'))
    } catch {
      setOrders(prev)
      setNotice('Echec de la suppression.')
    }
  }
  ...
  async function updateStatus(orderId: string, newStatus: OrderStatus) {
    ...
    try {
      ...
      setOrders(prev)
      setNotice('Echec de la mise a jour du statut.')
    }
  }
  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-600 text-lg">
          Gerez et suivez toutes les commandes clients en temps reel
        </p>
      </div>
    ...
    <input
      type="text"
      placeholder="Rechercher par client, ville, code postal..."
      value={query}
    ...
    />
      ...
      {sortedOrders.length === 0 ? (
        <EmptyState title="Aucune commande trouvee" description="Vous n'avez pas encore de commandes." />
      ) : (
       <>
    ...
    <table className="w-full">
      <thead>
        <tr className="border-b border-slate-200">
          ...
          <th className="py-3 ox-4 text-left text-slate-700 font-semibold text-base">Client</th>
          <th className="text-left py-3 px-4 text-sm font-medium text-slate-600">Date</th>
          <th className="text-left py-3 px-4 text-sm font-medium text-slate-600">Adresse</th>
  ...
  >
    ...
    {statusIcon(order.status as OrderStatus)}
              ...
              <td className="py-4 px-4 text-slate-800 font-medium">
                {order.total.toFixed(2)} {order.currency || 'DT'}
              </td>
