  ...
  return (
    <div className="space-y-6 p-6 md:p-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: '#111827' }}>
            {parent ? `Variants of ${parent.name}` : 'Produits'}
          </h1>
          <p className="mt-1 text-sm" style={{ color: '#6B7280' }}>
            {parent
              ? 'Gerez les variantes du produit selectionne'
              : 'Gerez votre catalogue produits'}
          </p>
        </div>
        <button
        ...
        >
          ...
          {parent ? 'Nouvelle variante' : 'Nouveau produit'}
        </button>
      </div>
      ...
      <p className="text-sm" style={{ color: '#9CA3AF' }}>
        {products.length === 0
          ? 'Aucun produit'
          : `${pageItems.length > 0 ? (page - 1) * perPage + 1 : 0}ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“${Math.min(page * perPage, products.filter(p => !categoryFilter || p.categories?.includes(categoryFilter)).length)} of ${products.filter(p => !categoryFilter || p.categories?.includes(categoryFilter)).length} product${products.length !== 1 ? 's' : ''}`}
      </p>

      {pageItems.length === 0 ? (
        <EmptyState
          title="Aucun produit trouve"
          description="Ajustez vos filtres ou ajoutez un nouveau produit"
        />
      ) : (
        <>
            ...
            <div className="flex items-center justify-center gap-2 pt-2">
              ...
              <div className="flex items-center gap-1">
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((p, i) =>
                    p === '...' ? (
                      <span key={`ellipsis-${i}`} className="px-2 text-sm" style={{ color: '#9CA3AF' }}>ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦</span>
                    ) : (
                      <button
                        key={p}
