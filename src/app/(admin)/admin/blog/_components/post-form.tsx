...
export default function PostForm({ action, initial, submitLabel }: PostFormProps) {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.set("Contenu", editorContent);
                ...
                {coverImagePreview ? (
                  <div className="mt-2 overflow-hidden rounded-xl border border-slate-200 dark:border-zinc-800 shadow-sm">
                    <div className="aspect-video w-full">
                      <img src={coverImagePreview} alt="Apercu de couverture" className="h-full w-full object-cover" />
                    </div>
                  </div>
                ) : null}
              </div>
          ...
          <section className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white/95 shadow-xl shadow-slate-200/40 dark:border-zinc-800 dark:bg-zinc-900/95">
            ...
            <div className="relative">
              <RichTextPlugin
                ...
                placeholder={
                  <div className="pointer-events-none absolute left-8 top-8 text-lg text-zinc-300">Commencez a rediger votre article...</div>
                }
...
function UnifiedTopNav({ submitLabel }: { submitLabel: string }) {
  ...
  const toggleLink = () => {
    const url = window.prompt("Saisir une URL (laisser vide pour supprimer) :", "https://");
    if (url === null) return;
