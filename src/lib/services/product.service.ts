...
function mapCategory(record: PocketBaseRecord): ShopCategory {
  ...
  const coverImage =
    typeof rawCover === 'string'
      ...
      : Array.isArray(rawCover) && rawCover.length > 0
        ...
        : null
  const rawMenuImage = record.menuImage
  const menuImage =
    typeof rawMenuImage === 'string'
      ? rawMenuImage.trim() || null
      : Array.isArray(rawMenuImage) && rawMenuImage.length > 0
        ? String(rawMenuImage[0]).trim() || null
        : null
  ...
  return {
    ...
    menuImage,
    menuImageUrl: menuImage ? fileUrl('categories', id, menuImage) : null,
    features,
  }
}
...
const getCachedCategories = unstable_cache(
  async (): Promise<ShopCategory[]> => {
    ...
    const records = await pb.collection('categories').getFullList(500, {
      sort: 'order,name',
      fields: 'id,name,slug,order,parent,desc,description,promo,activeAll,coverImage,menuImage,features',
      requestKey: null,
    })
...
/** Direct PocketBase lookup — bypasses unstable_cache, safe to use in dynamic routes. */
export async function getShopCategoryBySlugDirect(slug: string): Promise<ShopCategory | null> {
  const value = slug.trim()
  if (!value || !SLUG_REGEX.test(value)) return null
  try {
    const pb = createServerPb()
    const record = await pb.collection('categories').getFirstListItem(
      `slug="${escapePbString(value)}"`,
      { fields: 'id,name,slug,order,parent,desc,description,promo,activeAll,coverImage,menuImage,features', requestKey: null }
    )
    return mapCategory(record as PocketBaseRecord)
  } catch {
    return null
  }
}
