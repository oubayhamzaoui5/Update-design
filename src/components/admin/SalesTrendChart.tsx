...
export default function SalesTrendChart() {
  ...
  // Format currency for Tunisian Dinar (TND/DT)
  const formatCurrency = (val: number) => 
    new Intl.NumberFormat('fr-TN', { 
      style: 'currency', 
      currency: 'TND',
      minimumFractionDigits: 3 // Tunisian Dinar usually uses 3 decimal places
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500">
            {viewMode === 'month' ? `Ventes quotidiennes - ${MONTHS_FR[selectedMonth]}` : `Ventes mensuelles - ${selectedYear}`}
          </p>
        </div>
          ...
          <div className="flex items-center gap-4 ml-2 border-l pl-4 border-slate-200">
            ...
            >
              <div className="text-right">
                <p className="text-[10px] text-green-600 uppercase font-black tracking-wider">ventes totales</p>
      ...
      <div className="h-80 w-full relative">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data ?? []} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
            ...
            />
            <Tooltip 
              formatter={(value: number | undefined) => [formatCurrency(value ?? 0), "Ventes"]}
