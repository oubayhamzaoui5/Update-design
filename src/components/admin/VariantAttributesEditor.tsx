...
export default function VariantAttributesEditor({
  ...
  async function save() {
    try {
      ...
      setNotice('Attributs des variantes enregistres.')
      onSave?.()
    } catch (error) {
      console.error(error)
      setNotice("Echec de l'enregistrement des attributs.")
    } finally {
      setSaving(false)
  ...
  return (
    <div className="space-y-3 border-t border-slate-200 pt-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-slate-800">Attributs des variantes</h2>
