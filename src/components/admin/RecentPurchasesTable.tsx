...
type Status = 'pending' | 'confirmed' | 'delevering' | 'Livree' | 'cancelled' | 'En attente' | 'returned';
...
export default function RecentPurchasesTable() {
  ...
  const statusIcon = (status: Status) => {
    switch (status) {
      ...
      case 'Livree': return <ShoppingCart className="w-3 h-3" />
      case 'cancelled': return <Trash2 className="w-3 h-3" />
      case 'En attente': return <Pause className="w-3 h-3" />
  ...
  const statusBadgeClass = (status: Status) => {
    switch (status) {
      ...
      case 'Livree': return 'bg-emerald-50 text-emerald-700'
      case 'cancelled': return 'bg-red-50 text-red-700'
      case 'En attente': return 'bg-slate-100 text-slate-600'
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        console.error("Echec du chargement des achats recents", error);
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 mb-8 border border-slate-100">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-slate-800 font-semibold text-lg">Commandes recentes</h3>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-200">
              {['Client', 'Phone', 'Produits', 'Statut', 'Montant'].map((header) => (
