  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 mb-8 border border-slate-100">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-slate-800 font-semibold text-lg">Commandes Récentes</h3>
          <p className="text-sm text-slate-500">Aperçu des dernières transactions</p>
        </div>
        <Link 
          href="/admin/orders" 
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-200">
              {['Client', 'Téléphone', 'Produits', 'Statut', 'Montant'].map((header) => (
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              <tr><td colSpan={5} className="py-8 text-center text-slate-400 text-sm animate-pulse">Chargement...</td></tr>
            ) : purchases.length === 0 ? (
              <tr><td colSpan={5} className="py-8 text-center text-slate-400 text-sm">Aucune commande trouvée.</td></tr>
            ) : (
