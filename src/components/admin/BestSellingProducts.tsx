...
export default function BestSellingProducts() {
  ...
  // Helper for Tunisian Dinar formatting
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-xl shadow-sm p-6 mb-8 flex flex-col items-center justify-center min-h-[300px]">
        ...
        <p className="text-slate-500 text-sm italic">Calcul des statistiques en cours...</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
      ...
      <div className="grid grid-cols-[3fr_1fr_1.5fr] gap-4 mb-4 pb-2 border-b border-slate-50">
        <div className="text-slate-700 font-bold text-base">Meilleures Ventes</div>
        <div className="text-slate-500 text-sm font-medium text-center">Ventes</div>
        <div className="text-slate-500 text-sm font-medium text-right">Revenu (DT)</div>
      </div>
      ...
      <div className="space-y-1">
        {products.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 text-slate-400">
            ...
            <p className="text-sm italic">Aucune vente enregistree ces 7 derniers jours.</p>
          </div>
        ) : (
    ...
    <span className="text-xl">{product.image || 'ðŸ“¦'}</span>
      ...
      <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
        <button className="flex items-center gap-1 px-3 py-1.5 border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50 text-xs font-medium">
          7 derniers jours <ChevronDown className="w-4 h-4" />
        </button>
